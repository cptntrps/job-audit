...
| act | myapps `/jobs/` + the **pilot builder** (`portal/pilot_builder.py` in myapps; migration `work_002`) | per GBS function (`work.role`: the occupations each function is made of, seeded for HR / Workforce, License ops, GTS Infrastructure; lead names live only in the database), the split by importance-weighted share and the pilot candidates; the owner marks pilots started / done / dropped. |
